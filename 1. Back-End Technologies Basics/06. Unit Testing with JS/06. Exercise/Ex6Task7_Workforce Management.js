describe("Tests for workforceManagement object", function () {
  ...
  describe("recruitStaff", function () {
    it("should successfully recruit a developer with 4 or more years of experience", function () {
      ...
      expect(result).to.equal(
        "John has been successfully recruited for the role of Developer."
      );
    });
    ...
    it('should throw error when the role is not "Developer"', function () {
      expect(() =>
        workforceManagement.recruitStaff("John", "Designer", 5)
  ...
  describe("computeWages", function () {
    ...
    it("should throw an error for invalid hoursWorked input", function () {
      expect(() => workforceManagement.computeWages("160")).to.throw(
        "Invalid hours"
      );
  ...
  describe("dismissEmployee", function () {
    ...
    it("should throw an error for invalid workforce input", function () {
      expect(() =>
        workforceManagement.dismissEmployee("Not an array", 1)
      ).to.throw("Invalid input");
      expect(() =>
        workforceManagement.dismissEmployee([], "Invalid index")
      ).to.throw("Invalid input");
      expect(() => workforceManagement.dismissEmployee(["Petar"], -1)).to.throw(
        "Invalid input"
      );
      expect(() => workforceManagement.dismissEmployee(["Petar"], 2)).to.throw(
        "Invalid input"
      );
    });
  });
