describe("analyzeArray", function () {
  ...
  it("should return correct object with min, max, and length for an array of numbers", function () {
    expect(analyzeArray([1, 2, 3, 4, 5])).to.deep.equal({
      min: 1,
      max: 5,
      length: 5,
    });
    expect(analyzeArray([-5, -10, 0, 5])).to.deep.equal({
      min: -10,
      max: 5,
      length: 4,
    });
  ...
  it("should return correct object for an array with all equal elements", function () {
    expect(analyzeArray([3, 3, 3, 3])).to.deep.equal({
      min: 3,
      max: 3,
      length: 4,
    });
  ...
  it("should return correct object for an array with floating-point numbers", function () {
    expect(analyzeArray([1.1, 2.2, 3.3])).to.deep.equal({
      min: 1.1,
      max: 3.3,
      length: 3,
    });
